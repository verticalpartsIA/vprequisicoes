...
export const DigitalAttestationCard = ({ register, errors, ticketType }: DigitalAttestationCardProps) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="p-8 bg-brand/5 border border-brand/20 rounded-2xl shadow-md space-y-6">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-brand/10 rounded-2xl">
            <ShieldCheck className="w-8 h-8 text-brand" />
          </div>
          <div>
            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Ateste de Execução Digital</h3>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest italic">Confirmação de entrega para {typeLabel}</p>
          </div>
        </div>

        <div className="p-6 bg-white rounded-2xl border border-surface-border">
          <div className="flex items-start gap-4">
            <div className="pt-1">
              <input
                ...
                className="w-5 h-5 rounded border-surface-border bg-slate-50 text-brand focus:ring-brand"
              />
            </div>
            <label htmlFor="execution_confirmed" className="cursor-pointer">
              <span className="text-sm font-bold text-slate-700 block mb-1">Confirmo que o {typeLabel.toLowerCase()} foi executado conforme o pedido.</span>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block">Observações do Ateste</label>
          </div>
          <textarea
            {...register('notes')}
            placeholder="Descreva brevemente como foi a execução (ex: serviço concluído dentro do prazo, voo realizado etc.)"
            className={`w-full bg-white border ${errors.notes ? 'border-rose-500' : 'border-surface-border'} rounded-2xl px-5 py-4 text-sm text-slate-700 focus:ring-2 focus:ring-brand outline-none min-h-[120px] shadow-inner`}
          />
          {errors.notes && (
      ...
      <div className="p-6 bg-slate-50 border border-dashed border-surface-border rounded-2xl flex flex-col items-center justify-center text-center space-y-4">
          ...
          <button type="button" className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-[10px] font-bold text-slate-700 rounded-lg uppercase tracking-widest transition-colors">
            Selecionar Arquivos
          </button>
      </div>
    </div>
