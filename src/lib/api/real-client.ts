...
type StatusType   = Database['public']['Enums']['req_ticket_status'] | 'APPROVAL_L1' | 'APPROVAL_L2' | 'APPROVAL_L3';
...
export async function realGet(path: string, params?: Record<string, string>): Promise<any> {
  ...
  if (params?.status && path !== '/api/purchasing/tickets' && path !== '/api/receiving/tickets') {
...
export async function realPost(path: string, body: any): Promise<any> {
  ...
  if (quotationMatch) {
    ...
    // Atualizar Ticket para QUOTING ou PENDING_APPROVAL
    await supabase
      .from('req_tickets')
      .update({ 
        status: 'PENDING_APPROVAL',
        quoted_at: new Date().toISOString(),
        metadata: { ...body, quotation_id: quotation.id }
      })
      .eq('id', ticketId);
    ...
    await supabase.rpc('req_log_audit', {
      ...
      p_details: `Cotação do fornecedor ${body.supplier_id} registrada.`,
      p_level: 'success',
      p_module: 'V2_COTACAO'
    });
  ...
  if (approvalDecideMatch) {
    ...
    const totalValue = Number(ticket.metadata?.total_amount || 0);
